...
var utf_substring = function(arr, length) {
    var i = 0;
    var result = '';
    while (i < length) {
        result += arr[i];
...
function countChar(obj, feedback, limit, delbtn) {
    var limit = $('#'+limit).val();
    var splitter = new GraphemeSplitter();
    var textarea = $('#'+obj);
    var feedback = $('#'+feedback);
    var gr_array = splitter.splitGraphemes(textarea.val());
    var len = gr_array.length;
    if(delbtn) {
        if (len > 0) {
            $('#'+delbtn).show();
        } else {
    ...
    if (len > limit) {
        textarea.val(utf_substring(gr_array, limit));
    } else {
        var l = (limit - len);
        if(l < 10) {
        ...
        var thelen = sklonuj(l, $('#countChar_translation').val());
        feedback.text(thelen);
    }
}

var sklonuj = function(n, translation) {
    var p = translation.split('~');
    if(n == 0) {
        return p[3].replace('%i%', n);
    }
    if(n == 1) {
        return p[0].replace('%i%', n);
    }
    if(n > 1 && n < 5) {
        return p[1].replace('%i%', n);
    }
    if(n > 4) {
        return p[2].replace('%i%', n);
    }
}

var messageClear = function() {
    $('#messageSource').css('height', '192px').val('').focus();
    messageCountchar();
}

function messageCountchar() {
    countChar('messageSource', 'charCount_feedback', 'charCount_limit', 'charCount_delbtn');
}
$('#messageSource')
    .keyup(function() {messageCountchar()})
    .blur(function() {messageCountchar()})
    .change(function() {messageCountchar()})
    .focus(function() {messageCountchar()})
    .click(function() {messageCountchar()});

;(function($){
    //pass in just the context as a $(obj) or a settings JS object
    $.fn.autogrow = function(opts) {
        var that = $(this).css({resize: 'none'}) //prevent scrollies
        ...
        that.each(function(i, elem){
            ...
            if (opts.fixMinHeight) {
                elem.data('autogrow-start-height', 192); //set min height
            }
            elem.css('height', min);
        ...
        function resize (e){
            var box = $(this)
                ...
                , clone
            ;
            if (oldHeight < newHeight && newHeight < opts.maxHeight) { //user is typing
